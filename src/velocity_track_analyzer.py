            ...
            if "lapDTOs" in data:
                for lap in data["lapDTOs"]:
                    velocity_entries.append(
                        VelocityEntry(
                            float(lap["distance"]), float(lap["movingDuration"])
                        )
                    )
...
class VelocityTrackAnalyzer(object):
    def __init__(self, points: List[GPXTrackPoint], split_files: List[str] = None):
        ...
        self.distance_entries = [
            p.extensions_calculated.distance for p in self.points_with_time
        ]
        self.velocity_entries_from_garmin = get_velocity_entries_from_garmin(
            split_files
        )
        self.data = {}
    ...
    def get_average_velocity_for_kilometers(self, kilometer: int) -> float:
        ...
        for i, e in enumerate(self.velocity_entries_from_garmin):
            ...
            j = i
            while sum_kilometers < kilometer and j < len(
                self.velocity_entries_from_garmin
            ):
                sum_kilometers += self.velocity_entries_from_garmin[j].meter / 1000
                sum_duration_hours += (
                    self.velocity_entries_from_garmin[j].seconds / 3600
                )
                j += 1
            if sum_kilometers >= kilometer:
                velocities_in_kilometer_interval.append(
                    sum_kilometers / sum_duration_hours
                )
        return (
            max(velocities_in_kilometer_interval)
            if len(velocities_in_kilometer_interval)
            else 0.0
        )
        ...
        if len(self.distance_entries) == len(self.time_deltas):
            ...
            for entry in entries:
                if max(self.distance_entries) > entry.window_in_m:
                    ...
                    times = sums.loc[
                        (df.index >= to_datetime(entry.window_in_m, unit="s"))
                    ].values
                    if len(times) > 0:
                        velocity_from_track = entry.window_in_km * 3600 / times.min()
                        velocity_from_garmin = self.get_average_velocity_for_kilometers(
                            entry.window_in_km
                        )
                        self.data[entry.json_key_interval] = (
                            velocity_from_track
                            if velocity_from_track > velocity_from_garmin
                            else velocity_from_garmin
                        )
        return self.data
