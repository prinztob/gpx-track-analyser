...
def test_analyzing_track_with_errors():
    file = "resources/track_without_time.gpx"
    ...
    assert analyzer.vertical_velocities_60s == 0
    assert analyzer.vertical_velocities_600s == 0
    assert analyzer.vertical_velocities_3600s == 0
    assert abs(analyzer.slope_100 - 52.67) < 0.1
...
def test_analyzing_track6_gpx():
    ...
    analyzer.analyze()
    analyzer.get_maximal_values()
    assert abs(analyzer.slope_100 - 16.888) < 0.01
    assert abs(analyzer.vertical_velocities_60s - 0.29) < 0.01
    assert abs(analyzer.vertical_velocities_600s - 0.234) < 0.01
    assert abs(analyzer.vertical_velocities_3600s - 0.185) < 0.01
    assert analyzer.duration < 10

def test_analyzing_track6_full_gpx():
    file = "resources/track6_full.gpx"
