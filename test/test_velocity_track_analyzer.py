...
def test_reading_split_data() -> None:
    file = Path("resources/splits_1.json")
...
def test_max_velocity_in_kilometer_interval_merge_splits() -> None:
    analyzer = VelocityTrackAnalyzer(
        [],
        [
            Path("resources/activity_splits_4.json"),
            Path("resources/activity_splits_5.json"),
        ],
    )
...
def test_max_velocity_in_kilometer_interval_gpx_and_splits() -> None:
    track_analyzer = TrackAnalyzer(Path("resources/activity_with_splits.gpx"))
    ...
    track_analyzer_with_splits = TrackAnalyzer(
        Path("resources/activity_with_splits.gpx"),
        split_files=[Path("resources/activity_with_splits.json")],
    )
