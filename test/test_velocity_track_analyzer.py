...
def test_max_velocity_in_kilometer_interval_merge_splits():
    analyzer = VelocityTrackAnalyzer([], ["resources/activity_splits_4.json", "resources/activity_splits_5.json", ])
...
def test_max_velocity_in_kilometer_interval_gpx_and_splits():
    ...
    track_analyzer_with_splits = TrackAnalyzer("resources/activity_with_splits.gpx", split_files=["resources/activity_with_splits.json"])
    track_analyzer_with_splits.analyze()
    assert round(track_analyzer_with_splits.data["avg_velocity_1km"], 2) == round(track_analyzer.data["avg_velocity_1km"], 2)
